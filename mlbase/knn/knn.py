...
class KNearestNeighbor:
    ...
    def predict(self, X_test, num_loops=0):
        if num_loops == 0:
            distance = self.compute_distance_vectorized(X_test)
        elif num_loops == 1:
            distance = self.compute_distance_one_loop(X_test)
        else:
            distance = self.compute_distance_two_loop(X_test)
        return self.predict_labels(distance)

    def compute_distance_two_loop(self, X_test):
        num_test = X_test.shape[0]
        num_train = self.X_train.shape[0]
        distance = np.zeros(shape=(num_test, num_train))

        for i in range(num_test):
            for j in range(num_train):
                distance[i, j] = np.sqrt(np.sum((X_test[i, :] - self.X_train[j, :]) ** 2) + self.eps)
        return distance
    ...
    def compute_distance_one_loop(self, X_test):
        ...
        for i in range(num_test):
            distance[i, :] = np.sqrt(self.eps + np.sum((np.X_test[i, :] - self.X_train) ** 2, axis=1))
        return distance
