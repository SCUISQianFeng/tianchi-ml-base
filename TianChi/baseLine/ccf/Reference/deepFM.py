...
def merge_dense_input(dense_input_, embed_list, embedding_size, l2_reg):
    ...
    if len(dense_input) > 0:
        continuous_embedding_list = list(
            map(Dense(embedding_size, use_bias=False, kernel_regularizer=l2(l2_reg), ), dense_input))
...
def get_inputs_embedding(feature_dim_dict, embedding_size, l2_reg_embedding, l2_reg_linear, init_std, seed,
    ...
    if include_linear:
        linear_sparse_emb_dict = create_embedding_dict(feature_dim_dict, 1, init_std, seed, l2_reg_linear,
                                                       prefix + 'linear')
...
if __name__ == "__main__":
    ...
    data_offline = pd.read_csv(r'E:\DataSet\Tianchi\o2oSeason1\O2O_data\ccf_offline_stage1_train.csv')
    data_online = pd.read_csv(r'E:\DataSet\Tianchi\o2oSeason1\O2O_data\ccf_online_stage1_train.csv')
    test = pd.read_csv(r'E:\DataSet\Tianchi\o2oSeason1\O2O_data\ccf_offline_stage1_test_revised.csv')
    ...
    sns.barplot(x=data_offline[data_offline.label != -1]['label'].value_counts().index,
                y=data_offline[data_offline.label != -1]['label'].value_counts().values)

    feature = data_offline[(data_offline['Date'] < '20160516') | (
            (data_offline['Date'] == 'null') & (data_offline['date_received'] < '20160516'))].copy()
    dataset = data_offline[
        (data_offline['date_received'] >= '20160516') & (data_offline['date_received'] <= '20160615')].copy()
    fdf = feature.copy()
    # key of user 用户聚合特征
    ...
    # key of user and merchant 用户&商户聚合特征
    ...
    del data_offline, data_online, dataset, dataset2, dataset3
    gc.collect()

    dataset4 = reduce_mem_usage(dataset4)

    sparse_features = ['User_id', 'Merchant_id', 'Coupon_id', 'weekday', 'discount_type', 'weekday_type']
    dense_features = [fea for fea in dataset4.columns if
                      fea not in sparse_features and fea not in ['Discount_rate', 'Distance', 'date_received', 'Date',
                                                                 'label']]
